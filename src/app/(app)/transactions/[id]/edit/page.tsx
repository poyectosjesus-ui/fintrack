...
export default function EditTransactionMobile({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    api.get<any[]>('/api/categories').then((res) => {
      ...
      api.get<any>(`/api/transactions/${id}`).then((txRes) => {
        if (txRes.data) {
          setType(txRes.data.type);
          setAmount(txRes.data.amount.toString());
          setCategoryId(txRes.data.categoryId);
          setDescription(txRes.data.description);
        }
      });
    });
  }, [id]);
    ...
    <Dialog open={isOpen} onOpenChange={closeModal}>
      <DialogContent onClose={closeModal} className="overflow-hidden p-0 border-zinc-800">
        ...
        <div className="p-6 pb-2 border-b border-zinc-800/50 relative">
          ...
          <div className="flex justify-between items-start relative z-10">
            <DialogHeader>
              ...
              <DialogDescription>Modifica los datos de la transacción o elimínala.</DialogDescription>
            </DialogHeader>
            <Button
              variant="destructive"
              size="icon"
        ...
        <DialogFooter className="p-6 pt-2">
          <Button variant="outline" onClick={closeModal} disabled={loading || deleting} className="w-full sm:w-auto mt-2 sm:mt-0">
            Cancelar
          </Button>
          <Button 
          ...
          >
            {loading ? 'Actualizando...' : 'Terminar Edición'}
          </Button>
        </DialogFooter>
