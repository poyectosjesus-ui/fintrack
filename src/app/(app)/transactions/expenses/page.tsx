...
import { ArrowUpCircle, Plus, Search, ChevronLeft, ChevronRight, FileX } from 'lucide-react';
...
export default function ExpensesPage() {
  ...
  const fetchExpenses = async () => {
    setLoading(true);
    const res = await api.get<any>(`/api/transactions?type=EXPENSE&page=${page}&limit=12&search=${encodeURIComponent(search)}`);
    if (!res.error) {
  ...
  useEffect(() => {
    const delay = setTimeout(fetchExpenses, 300);
    ...
    <div className="w-full max-w-screen-2xl mx-auto px-4 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-zinc-800/60">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-400 flex items-center justify-center shadow-inner shrink-0">
            <ArrowUpCircle size={24} />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">Registro de Gastos</h1>
            <p className="text-sm text-zinc-400 font-medium mt-1 uppercase tracking-wider">Supervisión de Salidas</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
           ...
           <div className="relative">
              ...
              <Input 
                 ...
                 className="h-12 w-full sm:w-64 pl-10 bg-black border-zinc-800 rounded-xl focus-visible:ring-rose-500 text-sm font-medium" 
              />
           </div>
           {/* CTA */}
           <Link href="/transactions/new?type=EXPENSE">
             <Button className="h-12 px-6 font-bold rounded-xl shadow-md shadow-rose-500/10 bg-rose-600 hover:bg-rose-500 text-white w-full sm:w-auto transition-transform hover:-translate-y-0.5">
               <Plus size={18} className="mr-2" /> Extraer Fondos
             </Button>
           </Link>
        </div>
      </div>
           ...
           <table className="w-full text-left border-collapse">
              <thead>
                 <tr className="bg-zinc-950/50 border-b border-zinc-800/50 text-xs font-bold uppercase tracking-widest text-zinc-500">
                    ...
                    <th className="p-5 font-medium min-w-[200px]">Desglose del Gasto</th>
                    <th className="p-5 font-medium whitespace-nowrap">Responsable</th>
                    <th className="p-5 font-medium whitespace-nowrap">Categoría</th>
                    <th className="p-5 pr-6 font-medium text-right whitespace-nowrap">Extracción</th>
                 </tr>
              </thead>
                 ...
                 {loading ? (
                    <tr>
                       <td colSpan={5} className="p-16 text-center">
                          <span className="animate-spin rounded-full h-8 w-8 border-b-2 border-rose-500 mx-auto block mb-4"></span>
                    ...
                    <tr>
                       <td colSpan={5} className="p-16 text-center">
                          ...
                          <h3 className="text-zinc-300 font-bold text-lg">Historial Protegido</h3>
                          <p className="text-zinc-500 text-sm mt-1">No hay tickets de gastos en este bloque de búsqueda.</p>
                       </td>
                    </tr>
                 ) : (
                    expenses.map((tx: any) => (
                       <tr key={tx.id} className="hover:bg-zinc-900/40 transition-colors group">
                          ...
                          <td className="p-5 pr-6 whitespace-nowrap text-right">
                             <div className="flex flex-col items-end">
                                <span className="font-black text-rose-500 text-lg tracking-tight">-{formatMoney(tx.amount)}</span>
                                <span className="text-[10px] text-rose-500/70 font-bold uppercase tracking-widest">{tx.paymentMethod?.alias || 'Efectivo/General'}</span>
                             </div>
                          </td>
                       </tr>
                    ))
